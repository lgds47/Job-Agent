...
│   ├── job_skills.py            # LLM skill extraction for postings + backfill
│   ├── llm_json.py              # Robust JSON parsing for model output
│   ├── text_sanitize.py         # Strip accidental markdown fences from codegen
...
# 4. Set your Anthropic API key (either works)
#    a) Environment variable for the current shell:
export ANTHROPIC_API_KEY="sk-ant-..."
#    b) Or create a .env file in this directory (loaded automatically on startup):
#       ANTHROPIC_API_KEY=sk-ant-...

# 5. Set your target roles in the resume JSON
...
`python-dotenv` is optional at install time but listed in `requirements.txt` so
`.env` files work out of the box.
...
roles, and scores each posting against your resume on a 0–100 scale. After the
role filter, Claude extracts `required_skills` / `preferred_skills` from each posting
body so the `gaps` command can aggregate market demand from SQLite (older rows are
backfilled automatically when you run `gaps`).
...
any SQLite viewer or the Python API. `search` upserts scored jobs; `apply` records
the application folder path; `gaps` stores the latest planner triage snapshot in
`skill_gaps` after each run.
