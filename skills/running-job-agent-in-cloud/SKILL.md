...
| Gap planner/builder | `python orchestrator.py gaps --help` does not need the resume; running `gaps` does, and fails before DB checks if it is missing | With resume present, `python orchestrator.py gaps` prints "No jobs in state store yet" when `get_recent_jobs(n=50, min_score=50)` returns empty: either no job rows exist or all scores are below 50. Inspect `StateStore().summary()` if unclear. With qualifying jobs, it may call Claude to backfill missing skill lists before planning |
