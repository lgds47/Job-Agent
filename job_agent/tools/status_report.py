...
Self-contained — no third-party dependencies, no LLM calls. Safe to run
...
def collect_status(
    ...
    *,
    ...
    project_ideas = store.get_project_ideas()
    run_history = store.get_run_history(n=run_history_limit)
...
def _summarize_project_ideas(ideas: list[dict]) -> dict:
    ...
    for i in ideas:
        by_status[i.get("status") or "unknown"] = by_status.get(i.get("status") or "unknown", 0) + 1
...
def _summarize_run_history(runs: list[dict]) -> dict:
    ...
    for r in runs:
        ...
        totals["early_exits"] += int(r.get("early_exits") or 0)
        totals["claude_failures"] += int(r.get("claude_failures") or 0)
        if (r.get("status") or "").lower() == "error":
            totals["errors"] += 1
...
def format_text(status: dict) -> str:
    ...
    if ideas:
        rows = [["ID", "Gap", "Title", "Status", "Project Dir", "Updated"]]
        for i in ideas:
            rows.append([
                str(i.get("id", "")),
                _truncate(i.get("gap_skill") or "—", 18),
    ...
    if pdirs:
        rows = [["Title", "Status", "Skill", "Created", "Completed", "Dir"]]
        for p in pdirs:
            rows.append([
    ...
    if runs:
        rows = [["ID", "Cmd", "Started", "Status", "Scored", "Early", "Fails", "Notes"]]
        for r in runs:
            rows.append([
                str(r.get("id", "")),
                ...
                str(r.get("early_exits") or 0),
                str(r.get("claude_failures") or 0),
                _truncate(r.get("notes") or "—", 40),
            ])
        lines.append(_format_table(rows))
    else:
...
def format_html(status: dict) -> str:
    ...
    parts.append("</p>")
    parts.append(_html_table(
        ["ID", "Gap", "Title", "Status", "Project Dir", "Updated"],
        [
            [
                str(i.get("id", "")),
                i.get("gap_skill") or "—",
    ...
    parts.append(_html_table(
        ["Title", "Status", "Skill", "Created", "Completed", "Refined", "Dir"],
        [
            [
                p.get("title") or "—",
                p.get("status") or "—",
    ...
    parts.append(_html_table(
        ["ID", "Command", "Started", "Ended", "Status", "Scored", "Early", "Fails", "Notes"],
        [
            [
                str(r.get("id", "")),
                r.get("command") or "—",
                _short_dt(r.get("started_at"), with_time=True),
                _short_dt(r.get("ended_at"), with_time=True),
                r.get("status") or "—",
                str(r.get("jobs_scored") or 0),
                str(r.get("early_exits") or 0),
                str(r.get("claude_failures") or 0),
