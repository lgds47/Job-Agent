...
class SearchAgent:
    ...
    async def _discover_companies(self, roles: list[str]) -> list[dict]:
        ...
        companies = json.loads(_clean_json(response.content[0].text))
    ...
    async def _research_adhoc_company(self, company_name: str) -> dict:
        ...
        info = json.loads(_clean_json(response.content[0].text))
        return {
            "name": company_name,
    ...
    async def _fetch_custom(self, company: dict) -> list[dict]:
        ...
        try:
            ...
            listings = json.loads(_clean_json(response.content[0].text))
            return [
                {
                    **j,
                    "company": company["name"],
...
"""
        try:
            ...
            scoring = json.loads(_clean_json(response.content[0].text))
            return {**job, **scoring}
        except Exception as e:
            print(f"    ⚠️  Scoring failed ({job['title']}): {e}")
    ...
    async def run(
        ...
        Full pipeline:
          ...
          5. Score each against resume
          6. Persist and return sorted results

        Args:
            roles:            Target role titles (e.g. ["ML Engineer"])
            adhoc_companies:  Optional on-the-fly company names
                              e.g. ["Glean"] from `--company Glean` CLI flag
        ...
        filtered = [j for j in all_jobs if any(kw in j["title"].lower() for kw in keywords)]
        ...
        self.store.save_jobs(qualified)
